...
get '/date/:select_date' do
  ...
  @categories_sum = List.where(spent_date: params[:select_date]).where(user_id: user_id).find_by_sql("SELECT category_id FROM lists group by category_id").sum(:price)
